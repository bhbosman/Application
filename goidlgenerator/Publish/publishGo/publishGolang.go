...
func (self *publishGolang) Export(params Publish.ExportParams) error {
	...
	for _, declaredType := range params.DeclaredTypes {
		...
		if declaredType.Predefined() {
			...
			publish.Export(sb, params.TypeInformation)
			continue
		}

		if definition, ok := declaredType.(*yacc.StructDefinition); ok {
			publish := NewPublishStruct(definition, params.TypeInformation)
			publish.Export(sb, params.TypeInformation)
			continue
		}

		if definition, ok := declaredType.(*yacc.EnumDecl); ok {
			publish := NewPublishEnum(definition, params.TypeInformation)
			publish.Export(sb, params.TypeInformation)
			continue
		}
		if typeDeclarator, ok := declaredType.(*yacc.TypeDeclarator); ok {
			publish := NewpublishTypeDecl(typeDeclarator, params.TypeInformation)
			publish.Export(sb)
			continue
		}
	}

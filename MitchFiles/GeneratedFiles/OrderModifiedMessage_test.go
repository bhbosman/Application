...
func TestNewOrderModifiedMessage(t *testing.T) {
	t.Run("Constructor", func(t *testing.T) {
		msg, _ := OrderModifiedMessageFactory.New()
		assert.NotNil(t, msg)
	})
	...
	t.Run("WriteMessage", func(t *testing.T) {
		...
		n, err := Write_OrderModifiedMessage(mitchWriter, msg)
		...
		assert.NoError(t, err)
		assert.Equal(t, uint16(28), length)

		var messageByte byte
		err = binary.Read(reader, binary.LittleEndian, &messageByte)
		assert.NoError(t, err)
		assert.Equal(t, byte(0x55), messageByte)
	})
}

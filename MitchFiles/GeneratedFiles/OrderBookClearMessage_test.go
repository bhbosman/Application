...
func TestNewOrderBookClearMessage(t *testing.T) {
	t.Run("Constructor", func(t *testing.T) {
		msg, _ := OrderBookClearMessageFactory.New()
		assert.NotNil(t, msg)
	})
	...
	t.Run("WriteMessage", func(t *testing.T) {
		...
		n, err := Write_OrderBookClearMessage(mitchWriter, msg)
		...
		assert.NoError(t, err)
		assert.Equal(t, uint16(13), length)

		var messageByte byte
		err = binary.Read(reader, binary.LittleEndian, &messageByte)
		assert.NoError(t, err)
		assert.Equal(t, byte(0x79), messageByte)
	})
}
